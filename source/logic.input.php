...
// let's check just in case (JS didn't work or smth like that)
if (strlen($_POST['text']) > $maxsymbols) fallback("index.php?error=lang2");
...
if (($language == false) || ($language == null)) {
	...
	fallback("index.php?error=lang1");
}
if ($language[1][0] == "Russian") {
	if (isset($session)) $db_conn->exec("DELETE FROM sessions WHERE id='".$session."'");
	fallback("index.php?error=lang2");
}
	
// очистка текста от скверны
require_once "class.cleantext.php";
...
if ($intWordCount < $minwords) {
	if (isset($session)) $db_conn->exec("DELETE FROM sessions WHERE id='".$session."'");
	fallback("index.php?error=text1");
}
